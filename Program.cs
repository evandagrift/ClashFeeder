...
namespace ClashFeeder
{
    ...
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            var config = new ConfigurationBuilder().AddJsonFile("/var/www/codexupdater/appsettings.json", optional: false, reloadOnChange: true).Build();

            //db options builder
            var optionsBuilder = new DbContextOptionsBuilder<TRContext>();

            optionsBuilder.UseMySQL(config["ConnectionStrings:HostedLocalDBConnectionString"]);




            Client client = new Client(config["ConnectionStrings:HostedBearerToken"]);
            ...
            List<Card> cardsInDB = context.Cards.ToList();
            //Get Clan
            // Get all battles
