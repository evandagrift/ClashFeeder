...
namespace ClashFeeder.Repos
{
    public class PlayerSnapshotsRepo
    {
            ...
            try
            {
                ...
                if (result.IsSuccessStatusCode)
                {
                    ...
                    if (PlayerSnapshot.Name != "" && PlayerSnapshot.CurrentFavouriteCard != null)
                    {
                        ...
                        TrackedPlayer trackedPlayerSnapshot = _context.TrackedPlayers.Where(t => t.Tag == PlayerSnapshot.Tag).FirstOrDefault();
                        ...
                        {
                            _context.TrackedPlayers.Add(new TrackedPlayer { Tag = PlayerSnapshot.Tag, Priority = "high" });
                            _context.SaveChanges();
                        }
        ...
        {
            TrackedPlayer trackedPlayerSnapshot = _context.TrackedPlayers.Where(t => t.Tag == PlayerSnapshotTag).FirstOrDefault();
            ...
            if (trackedPlayerSnapshot == null)
            {
                _context.TrackedPlayers.Add(new TrackedPlayer { Tag = PlayerSnapshotTag, Priority = "high" });
                _context.SaveChanges();
            }
        ...
        {
            if (!string.IsNullOrEmpty(PlayerSnapshotTag))
            {
                BattlesRepo battlesRepo = new BattlesRepo(_client, _context);
